...
def draw(ax, S, d, title, sub, norm, cmap, ways, places, mark=True, q=90):
    ...
    for w in ways:
        p = np.array(w['pts'])
        gan = w.get('_gan')
        ax.plot(p[:, 1], p[:, 0], lw=1.7 if gan else 0.5,
                color='#ffffff' if gan else '#f2f2ef',
                alpha=.95 if gan else .7, zorder=3, solid_capstyle='round')
        ax.plot(p[:, 1], p[:, 0], lw=0.9 if gan else 0.25,
                color=INK2 if gan else MUTED, alpha=.85, zorder=3.1)
    # 高分区
    if mark:
        ...
        for i, c in enumerate(cl, 1):
            ...
            if c.get('place'):
                ...
                dy = (18 if fy < .5 else -18) + (i - 1) * (11 if fy < .5 else -11)
...
if __name__ == '__main__':
    ...
    fig.suptitle('哈尔滨形势评分：两套规则对照',
                 fontsize=18, color=INK, x=0.045, ha='left', y=0.975)
    fig.text(0.045, 0.940,
             '黑线圈出各自的前 10% 区域，编号见下表。粗白/灰线为「幹」（松花江、阿什河、拉林河、呼兰河、少陵河），细线为支水沟渠。'
    ...
    out = os.path.join(HERE, 'harbin_compare.png')
    fig.savefig(out, dpi=140, facecolor=SURFACE)
    print('→', out)

    json.dump(dict(modern=cl_m, zangjingyi=cl_z),
              open(os.path.join(HERE, 'top_areas.json'), 'w', encoding='utf8'),
