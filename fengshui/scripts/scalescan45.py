...
if __name__=="__main__":
    ...
    json.dump(res,open('out/scalescan45.json','w'),ensure_ascii=False,indent=1,default=float)
    KEY=['二里頭遺址','隋唐洛阳城遗址','东周王城','汉魏洛阳故城*','偃师商城*','白马寺','关林','洛南东汉帝陵','恭陵']
    print('\n' + '古迹'.ljust(16) + '4x'.rjust(8) + '5x'.rjust(8))
    for k in KEY:
        v=[res[s]['scores'].get(k) for s in (4.0,5.0)]
        if any(x is None for x in v): continue
        print(f"{k[:16]:18s}{v[0]:8.3f}{v[1]:8.3f}")
