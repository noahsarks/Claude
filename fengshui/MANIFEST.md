...
├── rules_luantou.yaml       规则登记册 v0.20，35 个顶层节，含全部结论与撤回记录
...
| **本地背景分位** | `typical/bg.py` | `typical/VALIDATION.md` | 每点在自己 15 km 邻域 400 随机点中的分位。已知被选中的八点里 5/8 高于本地中位，**三种检验全不显著**（p=0.363/0.134/0.219）。二里头 27%，长陵 97% |
...
`rules_luantou.yaml` 是本项目的**过程台账与唯一结论出口**，35 个顶层节，含每一次撤回。
...
- **「永定陵得低分正说明模型与五音姓利范式不匹配」** —— 这是我按原始分做的读法。
  算完本地背景分位后不成立：永定陵在自己 15 km 邻域里排 77 分位、z=+0.95。
...
12. **原始分不可跨区比较** —— 永定陵原始分 0.468 看着低，本地分位却是 77%、z=+0.95。
    低是巩义一带整体地形不如燕山，不是模型察觉到它被五音姓利选坏了。
    **本项目唯一一次预注册检验（写在 sites.yaml 里）因此判为不支持。**
13. **城址不能用逐点判据** —— 隋唐洛阳城跨洛河而建，取城南则洛河判在背后（「无水」），
    取城北则相反。**取点决定结论**，不是精度问题，是判据用错了对象。
    此前所有城址单点分都受影响；洛阳/关中的矛盾很可能有一部分出在这里。
14. **分位必须与背景离散度一起报** —— 长陵 97%（背景 SD 0.130）与上海中心 98%
    （背景 SD 0.021）数值几乎相同，证据价值完全不同。单报分位会把噪声读成信号。
15. **总分对单项改动不敏感** —— 删掉曾占 `hulong` 24.5% 的 `balance`，八点总分变化 |Δ| ≤ 0.036。
    含义：靠总分反推「哪条规则起了作用」做不到。
16. **明堂扇区的 ±45° 无原文依据且结论对它敏感** —— 七点里两点在 20°–90° 内翻档；
    「聚水」档八点全未触发。该项应连角差一起报告，不能只给二值判定。
