...
│   ├── river_check.py       **对 OSM waterway=river 校核汇流网络**（外部校核）
│   ├── drainage_effect.py   把「修 bug」与「改规则」的影响分开量
│   ├── sector_sweep.py      明堂扇区半角 20°–90° 敏感性扫描
│   ├── works.yaml           文献编目 22 条（题署／实际年代／可靠性／性质）
│   ├── corpus_stats.py      语料体量按性质与年代可靠性统计
│   ├── chronology_terms.py  按实际年代分九层的逐词计数（处理清代避讳）
│   ├── quote_verify.py      **把每条引文拿去语料里机械核对**
│   ├── term_trace.py        关键词在 30 卷语料中的出现次数（按卷）
...
| **语料结构统计** | `typical/corpus_stats.py` | `typical/CHRONOLOGY.md` | 技术文献 307,025 字中**托名 66.1%**，年代确定仅 17.2%（且全为明代）；批判文献 7 篇年代**100% 确定**。⇒ 年代可靠的部分主要是反对它的那部分 |
| **分层逐词计数** | `typical/chronology_terms.py` | `typical/CHRONOLOGY.md` | 按实际年代分九层。「擇向」只在 S4 出现此后皆 0；「支幹」集中在 S8 水龍經；「五音」最早见 S1 王充（约 80，且是驳论） |
| **引文机械核对** | `typical/quote_verify.py` | — | 47 条按引文核，42 条全部片段命中；据此改掉 5 处引文错误，其中 1 处是我给呂才补写的一句原文没有的话 |
| **关键词编年（1726 截止线）** | `typical/term_trace.py` `chronology_terms.py` | `typical/CHRONOLOGY.md` | 飛星／挨星／九運／運星／替卦／兼向／下卦／城門訣／令星 **各 0 次** ⇒「三元九運＋二十四山陰陽順逆＋挨星起星＋山向雙飛」这套排盘术是 1726 年以后的。**该结论已三次改窄**：查近义词（三元 19 次）、处理避讳（元空 18 次，名目宋元已有）、读原文（那两处是立向消水法与八宅装卦，都不是飞星） |
| **典型建筑 v0.6/v0.7 对照** | `typical/run.py` | `typical/VALIDATION.md` | 八点 \|Δ\| ≤ 0.036；长陵（唯一有形势派相地记载）七项全高 |
| **汇流网络对外部校核** | `typical/river_check.py` | `typical/VALIDATION.md` | 对 OSM `waterway=river`：真实河道到模型水道的中位距离，洛阳盆地 **2121→304 m**、燕山 **1106→212 m**。修前平原比山地差一倍，与「平地导流失效」的病因一致 |
| **bug 与规则的影响之比** | `typical/drainage_effect.py` | `typical/VALIDATION.md` | 同一套 v0.6 规则跑在修前/修后网络上：**修 bug 平均动 0.059，改规则动 0.025**——一个 bug 抵四条规则改动的 2.4 倍 |
| **本地背景分位** | `typical/bg.py` | `typical/VALIDATION.md` | 每点在自己 15 km 邻域 400 随机点中的分位。**v0.8：3/8 高于本地中位，分位均值 0.515（零假设 0.500），p=0.855/0.431/0.492——干净的零**。修前修后两批分位 Spearman 仅 0.287 |
...
- **「永定陵得低分正说明模型与五音姓利范式不匹配」** —— 这是我按原始分做的读法。
  算完本地背景分位后不成立：永定陵在自己 15 km 邻域里排 77 分位（v0.7 数，待重算）。
  原始分的低是区域基线差别（巩义 vs 燕山），不是模型察觉到那里被选坏了
- **所有城址的单点得分**（汉魏洛阳、隋唐洛阳、二里头，以及此前洛阳/关中格网里
  的城址样本）—— 取点决定结论，判据用错了对象。作为中间量保留，不作结论
- **此前一切涉水结论的数值**（洛阳 +0.564、关中 −1.400、三区分层效应 +0.043、
  水分量消融）—— 汇流网络在平地上是断的，真实河道到模型水道中位偏离 2.1 km。
  方向未必错，数值一律作废，须用 v0.8 网络重跑
- **「二里头本地 27 分位是唯一方向为负的结果」** —— v0.8 重算是 50%，
  正落在邻域中位上。那个 27% 是坏网络的产物。我当时还专门写了一段说它不能当噪声处理
- **「长陵是唯一七项全高的点」** —— v0.8 下水口 0.240（原 0.706），分位 97%→76%
- **拿「上海中心 98%」当分位失真的例子** —— 98% 本身也是坏网络造的（修后 48%）。
  「分位须与背景离散度并报」这条方法论结论保留，例子换掉
...
11. **引擎的汇流网络此前在平地上是断的** —— 形态学填洼造出严格平地，D8 要求严格正落差，
    平地上给不出方向；单瓦片 6.4% 内部格无下游，其中 100% 是这类平地。
    修复（Garbrecht–Martz 双梯度 + BFS 兜底）后对 OSM 河道外部校核：
    洛阳盆地中位偏离 2121→304 m，燕山 1106→212 m。
    **一个 bug 对分数的影响（0.059）是四条规则改动加起来（0.025）的 2.4 倍。**
    教训：这个 bug 从 v0.1 存到 v0.7，期间做过五点验证、三区分层、尺度扫描、
    噪声测量、消融实验，没有一项检出它——因为每一项都只比较模型自己的输出。
    **凡引擎算出的地理要素，都要有一次对外部数据的位置校核。**
12. **「坐向度数 → 挨星起星 → 山盘向盘双飞」这套排盘术是 1726 年以后的** —— 语料实证
...
13. **原始分不可跨区比较** —— 分数必须放回它自己的邻域读。永定陵按原始分看着低，
    本地分位却是 **84%，是八点里最高的**（v0.8）——低是巩义一带整体地形不如燕山，
    是区域基线差别，不是模型察觉到它被五音姓利选坏了。
14. **八个已知被选中的点，在自己邻域里的分位均值 0.515** —— 零假设 0.500。
    不是「不显著」，是恰好落在随机上（3/8 高于本地中位，p=0.855/0.431/0.492）。
    坏网络时是 0.615，也不显著；修好之后连那点虚高也没了。
15. **修前修后两批分位的 Spearman 只有 0.287，平均绝对变化 24 个百分点** ——
    同样八个点只差一个 bug 修复。旧那批几乎不含信息；在有别的东西来校之前，
    新那批自己的可靠性同样未知。
16. **城址不能用逐点判据** —— 隋唐洛阳城跨洛河而建，取城南则洛河判在背后（「无水」），
    取城北则相反。**取点决定结论**，不是精度问题，是判据用错了对象。
    此前所有城址单点分都受影响；洛阳/关中的矛盾很可能有一部分出在这里。
17. **分位必须与背景离散度一起报** —— 陆家嘴两点的邻域在 DEM 里是一块平板
    （背景 SD 0.020/0.021，其余各点 0.11–0.14），几厘的差就能推出几十个分位。
18. **总分对单项改动不敏感** —— 删掉曾占 `hulong` 24.5% 的 `balance`，八点总分变化 |Δ| ≤ 0.036。
    含义：靠总分反推「哪条规则起了作用」做不到。
19. **明堂扇区的 ±45° 无原文依据且结论对它敏感** —— 修好汇流网络后重扫，
    七点里**五点**在 20°–90° 内翻档（修之前只有两点，因为那时水系根本没找到）。
    该项应连角差一起报告，不能只给档位。
20. **《水龙经》平洋法已补（B7）** —— 支干相扶、干水回头环绕、屈曲（三回五度）、
    澄清停蓄，另加门槛级凶格「干水散气」。平洋模式下玄武/护龙归零，势形改用干/支
    （「以幹龍繞抱，取外氣形局；以支龍正息交會，取內氣孕育」是原话，不是类比）。
    干水判不出时不判，权重摊回其余各项。
21. **在这部官修类书里，年代可靠的部分主要是反对它的那部分** —— 技术文献 307,025 字中
    托名 66.1%、年代确定仅 17.2%（且全为明代）；批判文献 7 篇年代 100% 确定，
    全书年代最早且确定的一段是东汉王充的驳论。
    **直接后果：「A 级=最可信」是错的读法**——A 级五条全落在托名层，
    只表示「此后没再变过」，不表示来源可靠。
22. **五音姓利在东汉已被驳，此后又用了一千年** —— 王充《論衡·詰術篇》（约 80）
    引其说而「詰曰」驳之，呂才 640 年再驳，北宋皇陵在第二次驳斥后仍按它定向四百年。
    **一套方法被证伪，与它被废弃，之间没有必然联系**——这是对「找出正确规则就够了」
    最直接的反例。
23. **词频只能用来排除，不能用来确认** —— 玄空那条结论三次机械核对三次改窄：
    没查近义词、没处理清代避讳（玄→元）、只数词没读原文。数出 0 可以说「没有」，
    数出 n>0 必须去读。
24. **《五星捉脈正變明圖》（宋元）已按「高山／平岡／平地」分三格论穴** ——
    语料里最早的按地形分层，比本项目的 `mode` 早七百年，而本项目只分两格。
25. **「形势派 vs 理气派」在原始文本里并不干净** —— 《水龙经》卷 671 總論一说
    「水龍作用，全在八卦三元」，同卷《自然水法》末句却说「何用九星並八卦，
    生旺死絕皆虛說」。前者是编次者的话，后者是所收口诀本身，态度正相反。
