...
├── rules_luantou.yaml       规则登记册 v0.21，38 个顶层节，含全部结论与撤回记录
...
| **本地背景分位** | `typical/bg.py` | `typical/VALIDATION.md` | 每点在自己 15 km 邻域 400 随机点中的分位。v0.7 下八点 5/8 高于本地中位、三种检验全不显著（p=0.363/0.134/0.219）；**该批数值随汇流 bug 一并作废，正用 v0.8 重算** |
...
`rules_luantou.yaml` 是本项目的**过程台账与唯一结论出口**，38 个顶层节，含每一次撤回。
...
- **此前一切涉水结论的数值**（洛阳 +0.564、关中 −1.400、三区分层效应 +0.043、
  水分量消融、v0.20 的八点分位）—— 汇流网络在平地上是断的，真实河道到模型水道
  中位偏离 2.1 km。方向未必错，数值一律作废，须用 v0.8 网络重跑
...
13. **原始分不可跨区比较** —— 分数必须放回它自己的邻域读。v0.7 下永定陵原始分 0.468
    看着低，本地分位却是 77%——低是巩义一带整体地形不如燕山，是区域基线差别。
    这条方法论结论成立；**具体数值随汇流 bug 作废，正在重算。**
14. **城址不能用逐点判据** —— 隋唐洛阳城跨洛河而建，取城南则洛河判在背后（「无水」），
    取城北则相反。**取点决定结论**，不是精度问题，是判据用错了对象。
    此前所有城址单点分都受影响；洛阳/关中的矛盾很可能有一部分出在这里。
15. **分位必须与背景离散度一起报** —— v0.7 下长陵 97%（背景 SD 0.130）与上海中心 98%
    （背景 SD 0.021）数值几乎相同，证据价值完全不同：后者的邻域在 DEM 里是一块平板，
    0.03 的差就能推到 98 分位。方法论结论成立，数值待重算。
16. **总分对单项改动不敏感** —— 删掉曾占 `hulong` 24.5% 的 `balance`，八点总分变化 |Δ| ≤ 0.036。
    含义：靠总分反推「哪条规则起了作用」做不到。
17. **明堂扇区的 ±45° 无原文依据且结论对它敏感** —— 修好汇流网络后重扫，
    七点里**五点**在 20°–90° 内翻档（修之前只有两点，因为那时水系根本没找到）。
    该项应连角差一起报告，不能只给档位。
18. **《水龙经》平洋法已补（B7）** —— 支干相扶、干水回头环绕、屈曲（三回五度）、
    澄清停蓄，另加门槛级凶格「干水散气」。平洋模式下玄武/护龙归零，势形改用干/支
    （「以幹龍繞抱，取外氣形局；以支龍正息交會，取內氣孕育」是原话，不是类比）。
    干水判不出时不判，权重摊回其余各项。
19. **「形势派 vs 理气派」在原始文本里并不干净** —— 《水龙经》卷 671 總論一说
