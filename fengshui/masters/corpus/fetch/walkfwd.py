...
def prev_of(h):
    m = re.search(r'后一篇[：:]?</span><a href="([^"]+)"', h)
...
url = 'http://blog.sina.com.cn/s/blog_537afff00102yjqt.html'
found, steps = {}, 0
while url and steps < 400:
    h = get(url)
    if not h: break
    ti = title_of(h)
    ...
    steps += 1
json.dump(found, open('/home/user/ms/shen_found_fwd.json','w'), ensure_ascii=False, indent=1)
